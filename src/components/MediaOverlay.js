...
function LocalPreview({ stream }) {
  ...
  return _jsxs("div", {
    className:
      "fixed bottom-4 right-4 z-50 w-44 rounded-lg overflow-hidden border border-blue-400 shadow-xl",
    style: { backgroundColor: "rgba(137, 207, 240, 0.8)" },
    children: [
      _jsx("video", {
        ref: ref,
      ...
      _jsx("div", {
        className: "px-2 py-1 text-[10px] text-stone-300 font-mono",
        children: "you",
      }),
    ],
  });
...
export default function MediaOverlay({
  ...
  return _jsxs(_Fragment, {
    children: [
      _jsxs("div", {
        className:
          "fixed top-4 left-4 z-50 bg-transparent backdrop-blur-sm border border-stone-700/40 shadow-lg rounded-xl px-6 py-3 flex items-center gap-6 font-mono text-xs select-none",
        children: [
          _jsx("button", {
            onClick: onToggleMic,
            ...
            children: micMuted ? "mic muted" : "mic on",
          }),
          _jsx("button", {
            ...
            children: videoEnabled ? "camera on" : "camera off",
          }),
          _jsxs("label", {
            className:
              "flex items-center gap-2 text-black uppercase tracking-widest",
            children: [
              "video size",
              _jsx("input", {
                type: "range",
      ...
      _jsx("div", {
        ...
        children: remoteList.map((player) =>
          _jsx(
            RemoteVideoTile,
            {
              player: player,
      ...
      _jsx(LocalPreview, { stream: localStream }),
    ],
  });
}
