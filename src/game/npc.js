...
export function createNPCs(canvasWidth, canvasHeight) {
    return NPC_SPRITES.map((sprite, i) => {
        const zone = SPAWN_ZONES[i % SPAWN_ZONES.length];
        const spawnX = canvasWidth * (zone.xMin + Math.random() * (zone.xMax - zone.xMin));
        const spawnY = canvasHeight * (zone.yMin + Math.random() * (zone.yMax - zone.yMin));
        return {
            x: spawnX,
            y: spawnY,
            width: NPC_SIZE,
...
export function updateNPCs(npcs, dt, _canvasWidth, _canvasHeight, solids) {
