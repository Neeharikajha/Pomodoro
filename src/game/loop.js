...
function getZoom(viewportWidth, viewportHeight) {
    const ratio = Math.min(viewportWidth / BASE_VIEW_WIDTH, viewportHeight / BASE_VIEW_HEIGHT);
    return Math.max(0.8, Math.min(1, ratio));
}
export function startLoop(canvas, onStateChange, netClient, getRemotePlayers, character = "Boy1", onViewChange) {
    ...
    const tick = (timestamp) => {
        ...
        camera.x = Math.max(0, Math.min(player.x + player.width / 2 - visibleWorldWidth / 2, WORLD_WIDTH - visibleWorldWidth));
        camera.y = Math.max(0, Math.min(player.y + player.height / 2 - visibleWorldHeight / 2, WORLD_HEIGHT - visibleWorldHeight));
