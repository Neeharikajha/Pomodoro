...
export function useRoomMedia(localPlayerId, netClient, remotePlayers) {
    ...
    const knownRemoteIdsRef = useRef(new Set());
    ...
    const handleIncomingSignal = useCallback(async (signal) => {
        ...
        if (payload.kind === "offer") {
            const offerCollision = makingOfferRef.current.get(remoteId) || pc.signalingState !== "stable";
            ignoreOfferRef.current.set(remoteId, !polite && !!offerCollision);
            if (ignoreOfferRef.current.get(remoteId))
                return;
            if (offerCollision) {
                try {
